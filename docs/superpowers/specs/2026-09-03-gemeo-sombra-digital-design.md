...
- **Saúde:** `/healthz` por fonte (último ciclo, idade, cobertura, requisições SunOp hoje ÷ teto), último `modelar` e duração, banco alcançável. Monitor externo → Teams. O gêmeo renova o próprio token SunOp e alarma vencimento com 2 dias.
