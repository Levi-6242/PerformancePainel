...
## Fase E — Plataforma e operação

### Tarefa 20: Proxy `/gemeo/*` na plataforma e entrada "Gêmeo Digital" no menu

**Files:**
- Modify: `plataforma/app.py` — logo depois de `def logout()` (bloco de autenticação, ~linha 395): a rota `gemeo_proxy`
- Modify: `docs/redesign/Monitoramento (novo design).html` — três pontos: o `<a>` no `<nav>` (linha ~1863), uma regra CSS após `.nav a[aria-current="page"]` (linha ~45) e um `fetch` antes de `render();` no fim do arquivo
- Test: `tests/test_gemeo_proxy.py` (suíte da RAIZ: o `conftest.py` põe `plataforma/` no path e `import app` funciona como nos outros testes)
...
Run: `python -m pytest tests/test_gemeo_proxy.py -q` (na raiz)
Expected: FAIL — 404 em todas (a rota não existe).

- [ ] **Step 3: Acrescentar o proxy ao `plataforma/app.py`**

Logo após o bloco (âncora única):
...
- [ ] **Step 4: Entrada no menu do redesign (nasce escondida)**

No `<nav>` (linha ~1863), depois de `id="nav-tokens">Tokens</a>`, acrescentar:

```html
<a href="/gemeo/" target="gridco-gemeo" rel="noopener" id="nav-gemeo">Gêmeo Digital</a>
```

No CSS, depois de `.nav a[aria-current="page"]{...}` (linha ~45):

```css
/* Gêmeo Digital: a entrada só aparece quando o proxy /gemeo responde (fetch no fim do arquivo) */
html:not(.gemeo-on) #nav-gemeo{display:none}
```

Antes de `render();\nloadCurrent();\n</script>` no fim do arquivo:

```javascript
// Gêmeo Digital (projeto separado, proxy /gemeo/* no app.py): a entrada do menu só aparece quando o
// serviço responde — 200 ou 503 (healthz com problema ainda é o gêmeo falando); 404 = plataforma sem
// o proxy (versão antiga no ar) e o menu fica como está.
fetch('/gemeo/healthz', {cache: 'no-store'}).then(function (r) {
  if (r.status === 200 || r.status === 503) document.documentElement.classList.add('gemeo-on');
}).catch(function () {});
```

- [ ] **Step 5: Rodar e ver passar; conferir que a plataforma no ar não mudou**

Run: `python -m pytest tests/test_gemeo_proxy.py -q` → 4 passed. Depois `python -m pytest -q` da raiz.
Conferir: `curl -s -o NUL -w "%{http_code}" http://127.0.0.1:5050/gemeo/healthz` → **404** (processo antigo, sem o proxy — esperado até a T.I. reiniciar) e a tela `/` continua abrindo sem a entrada.

- [ ] **Step 6: Commit**

```bash
git add plataforma/app.py "docs/redesign/Monitoramento (novo design).html" tests/test_gemeo_proxy.py
git commit -m "feat(plataforma): proxy /gemeo/* para o Gemeo Digital e entrada de menu condicionada ao healthz (Tarefa 20)"
```
