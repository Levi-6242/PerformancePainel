...
def _gerencial_payload(force=False, ano=None, mes=None):
    ...
    for k, tp in (_thopen_prod_mtd() if atual else _thopen_prod_build(alvo_a, alvo_m)).items():
...
def _strings_curva_longo(fonte, usina, data_iso):
    ...
    if fonte == "pv":
        token = get_token()
        try:
            nome_api = next((p["nome"].strip() for p in get_plants(token) if str(p["id"]) == str(usina)), "")
        except Exception:
            nome_api = ""
        pid = int(usina)
        hoje = datetime.now().strftime("%Y-%m-%d")
...
def api_strings_curva_csv(fonte):
    ...
    Trancadas já excluídas (mesmos builders da tela). Vale p/ pv/pg/sunop/axis/owen."""
    import csv as _csv
    import io as _io
    if fonte not in ("pv", "pg", "sunop", "axis", "owen"):
...
def _cache_load():
    ...
    if pg.get("summary") is not None:
        _pg_cache.update({"summary": pg["summary"], "detail": _int_keys(pg.get("detail")),
                          "ts": pg.get("ts", 0.0)})
        n += 1
    # EXTRAS (formas próprias) — restaura só o que veio preenchido; o TTL de cada um decide se refaz
