...
(277,68 x 0,86 x 0,96), sem teto. Com teto de 200 kW, 200. Zero POA da zero."""
import numpy as np
import pandas as pd
import pytest
...
def test_ponto_de_referencia_stc():
    p = esperado.ParamsModelo(kwp=277.68, pac0_kw=1e9)
    s = esperado.esperado_inversor(pd.Series([1000.0, 0.0]), pd.Series([25.0, 25.0]), p)
    assert s.iloc[0] == pytest.approx(277.68 * 0.86 * 0.96, rel=1e-3) and s.iloc[1] == 0.0
...
def test_inferir_pac0_prefere_a_placa_e_marca_quando_infere():
    ...
    assert v == pytest.approx(200.0) and inf is True
