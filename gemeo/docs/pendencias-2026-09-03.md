...
| # | Pendência | O que é preciso |
|---|---|---|
| G1 | Workbook **`gemeo_digital`** (id 36) criado na API da Performance com 7 abas (`usina`, `equipamento`, `alias`, `modelo`, `cascata_dia`, `perda_dia`, `evento`) e carregado com a rodada real de **01/09** da MRO100 e da Santarém 1 (739 linhas). Foi carga MANUAL (script no scratchpad da sessão) | O `modelar` ainda não publica sozinho: falta um passo "publicar" no job. **Caminho que funciona:** gerar um xlsx (linha 1 = cabeçalho, célula vazia = `None`, nunca `""`) e `POST /api/workbooks/gemeo_digital/sync-xlsx?replace=true` — é o único jeito que grava cabeçalhos; `POST .../sheets` com `headers` e `PUT rows` com `headers` são ignorados pela API e as linhas viram "Coluna N". `leitura` e `esperado` continuam só no PostgreSQL (volume; a API não tem consulta por período). |
